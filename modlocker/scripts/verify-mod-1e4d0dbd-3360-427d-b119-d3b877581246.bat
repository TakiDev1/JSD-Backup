...
set "LICENSE_KEY=1e4d0dbd-3360-427d-b119-d3b877581246"
set "MOD_DOWNLOAD=http://localhost:3000/mods/download/1e4d0dbd-3360-427d-b119-d3b877581246"
set "PS_SCRIPT=license-check-1e4d0dbd-3360-427d-b119-d3b877581246.ps1"
set "LOGFILE=log.txt"
set "LOCK_FILE=%LOCALAPPDATA%\BeamNG.drive\0.36\settings\editor\1e4d0dbd-3360-427d-b119-d3b877581246.json"

:: Initialize log file
(
  echo === BeamNG Mod Locker Log ===
  echo Timestamp: %DATE% %TIME%
  echo License Key: 1e4d0dbd-3360-427d-b119-d3b877581246
  echo PS Script: %PS_SCRIPT%
  echo Mod URL: http://localhost:3000/mods/download/1e4d0dbd-3360-427d-b119-d3b877581246
...
echo. >> "%LOGFILE%"
echo [*] Starting download from: http://localhost:3000/mods/download/1e4d0dbd-3360-427d-b119-d3b877581246 >> "%LOGFILE%"

powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "try { $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri 'http://localhost:3000/mods/download/1e4d0dbd-3360-427d-b119-d3b877581246' -OutFile 'active_mod.zip' -UseBasicParsing -TimeoutSec 60; Write-Output '[√] Download completed successfully'; exit 0 } catch { Write-Output ('[ERROR] Download failed: ' + $_.Exception.Message); exit 1 }" >> "%LOGFILE%" 2>&1
...
echo Downloaded=1e4d0dbd-3360-427d-b119-d3b877581246 > "%LOCK_FILE%"
if exist "%LOCK_FILE%" (
