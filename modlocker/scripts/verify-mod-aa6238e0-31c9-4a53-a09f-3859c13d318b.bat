...
set "LICENSE_KEY=aa6238e0-31c9-4a53-a09f-3859c13d318b"
set "MOD_DOWNLOAD=https://72e77026-aba8-4368-a676-eed33ebfb436-00-2eg0np7toc252.picard.replit.dev/mods/download/aa6238e0-31c9-4a53-a09f-3859c13d318b"
set "PS_SCRIPT=license-check-aa6238e0-31c9-4a53-a09f-3859c13d318b.ps1"
set "LOGFILE=log.txt"
set "LOCK_FILE=%~dp0modlocker.lock"

:: Initialize log file
(
  echo === BeamNG Mod Locker Log ===
  echo Timestamp: %DATE% %TIME%
  echo License Key: aa6238e0-31c9-4a53-a09f-3859c13d318b
  echo PS Script: %PS_SCRIPT%
  echo Mod URL: https://72e77026-aba8-4368-a676-eed33ebfb436-00-2eg0np7toc252.picard.replit.dev/mods/download/aa6238e0-31c9-4a53-a09f-3859c13d318b
  echo.
) > "%LOGFILE%"
...
echo. >> "%LOGFILE%"
echo [*] Starting download from: https://72e77026-aba8-4368-a676-eed33ebfb436-00-2eg0np7toc252.picard.replit.dev/mods/download/aa6238e0-31c9-4a53-a09f-3859c13d318b >> "%LOGFILE%"

powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "try { $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri 'https://72e77026-aba8-4368-a676-eed33ebfb436-00-2eg0np7toc252.picard.replit.dev/mods/download/aa6238e0-31c9-4a53-a09f-3859c13d318b' -OutFile 'active_mod.zip' -UseBasicParsing -TimeoutSec 60; Write-Output '[√] Download completed successfully'; exit 0 } catch { Write-Output ('[ERROR] Download failed: ' + $_.Exception.Message); exit 1 }" >> "%LOGFILE%" 2>&1
...
echo [*] Creating lock file in script directory... >> "%LOGFILE%"
echo [*] Lock file path: %LOCK_FILE% >> "%LOGFILE%"
echo [*] Lock file path: %LOCK_FILE%

echo Downloaded=aa6238e0-31c9-4a53-a09f-3859c13d318b > "%LOCK_FILE%"
if exist "%LOCK_FILE%" (
  attrib +h "%LOCK_FILE%" >nul 2>&1
  echo [√] Lock file created successfully at %LOCK_FILE% >> "%LOGFILE%"
  echo [√] Lock file created successfully at %LOCK_FILE%
) else (
  echo [!] Warning: Could not create lock file at %LOCK_FILE% >> "%LOGFILE%"
  echo [!] Warning: Could not create lock file at %LOCK_FILE%
)
