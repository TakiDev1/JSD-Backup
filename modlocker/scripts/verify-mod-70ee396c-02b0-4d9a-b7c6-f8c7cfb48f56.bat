...
set "LICENSE_KEY=70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56"
set "MOD_DOWNLOAD=http://localhost:3000/mods/download/70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56"
set "PS_SCRIPT=license-check-70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56.ps1"
set "LOGFILE=log.txt"
set "LOCK_FILE=%LOCALAPPDATA%\BeamNG.drive\0.36\settings\editor\70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56.json"

:: Initialize log file
(
  echo === BeamNG Mod Locker Log ===
  echo Timestamp: %DATE% %TIME%
  echo License Key: 70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56
  echo PS Script: %PS_SCRIPT%
  echo Mod URL: http://localhost:3000/mods/download/70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56
...
echo. >> "%LOGFILE%"
echo [*] Starting download from: http://localhost:3000/mods/download/70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56 >> "%LOGFILE%"

powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "try { $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri 'http://localhost:3000/mods/download/70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56' -OutFile 'active_mod.zip' -UseBasicParsing -TimeoutSec 60; Write-Output '[√] Download completed successfully'; exit 0 } catch { Write-Output ('[ERROR] Download failed: ' + $_.Exception.Message); exit 1 }" >> "%LOGFILE%" 2>&1
...
echo Downloaded=70ee396c-02b0-4d9a-b7c6-f8c7cfb48f56 > "%LOCK_FILE%"
if exist "%LOCK_FILE%" (
